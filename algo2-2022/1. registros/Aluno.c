...
//funcao que escreve o registro no arquivo
void writeDataOnFile(FILE* filePointer, Aluno aluno){
    fwrite(&aluno.nusp, sizeof(int), 1, filePointer);
    fwrite(&aluno.nome_completo, sizeof(char), NOME_LEGNTH, filePointer);
    fwrite(&aluno.curso, sizeof(char), CURSO_LENGTH, filePointer);
    fwrite(&aluno.nota, sizeof(float), 1, filePointer);
}
